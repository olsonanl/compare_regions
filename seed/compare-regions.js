define([
    ...
    'dojox/timing',
    'dojo/domReady!'
], function(Evented, declare, gfx, gfx_utils, Tooltip, dom, on, connect, domStyle, lang,
	    popup, TooltipDialog,
	    Menu, MenuItem, PopupMenuItem,
	    timing
	   ) {

    return declare("seed.CompareRegions", [Evented], {
...
/*
	    on(dom.byId("compare_save",), 'click', function() {
		
	    });
*/

/*
	    on(dom.byId("compare_zoom_in"), 'click', function() {
		console.log("zoom in");
...
//	    });
	...
	render: function() {
		...
		if (typeof(max_offset) === "number")
		{
		    ...
		    min_offset = Math.min(min_offset, left_offset);
		}
		else
		{
		...
		endpoints.sort(function(a, b) { return a[1] < b[1] ? -1 : ((a[1] > b[1]) ? 1 : 0); });
		var depth = 0;
		...
		endpoints.forEach(function (elt) {
			...
			if (iend - ibeg > 0)
			{
			    ...
			    var feature = { fid: "intergenic" + intergenic_count++,
					    beg: ibeg,
					    end: iend,
					    size: iend - ibeg + 1,
					    strand: '+',
		...
		track_params[rowi] = { min : track_min,
				       max : track_max,
				       center: center,
				       mirror: mirror,
	...
	add_feature: function(row, row_data, x1, x2, height, color, feature, is_pin) {
		...
		around: { x: Math.round(bb[0].x + this.container.offsetLeft),
	...
	add_row: function(idx, data, track_info, row_idx) {
	    ...
	    var label_txt = this.name_group.createText({ text: label,
